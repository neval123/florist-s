...
//        		buyFlower.setEnabled(true);
    ...
    //    add(new Paragraph("It’s a place where you can grow your own UI 🤗"));
        ...
        /*	if(checked == true) {
        		transactions = transactionService.getTransactions();
        		Transaction t = transactions.stream().max(Comparator.comparing(Transaction::getTransactionNumber)).orElse(null);
        		currentPrimary = t.getTransactionNumber()+1;
    		}else {
    			currentPrimary = 1;
    		}*/
//       	int amount = (int)Math.round(flowerAmount.getValue());
 ...
 //       		flowerService.setFlowerAmount(f.getAmount() - amount, f.getId());
        		...
        		flowersToBuy.clear();
        		amounts.clear();
        //		transactions.stream().max(Comparator.comparing(Transaction::getTransactionNumber)).orElse(null);
        //		currentPrimary = transactions.get(0).getTransactionNumber();
        		positions = 0;
        	}
        });
//        setSizeFull();
